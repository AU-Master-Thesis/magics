...
impl BoundingBox3d {
    ...
    pub fn from_center_and_half_extents(center: Vector3<f32>, x: f32, y: f32, z: f32) -> Result<Self, NegativeExtentsError> {
    ...
    pub fn volume(&self) -> f32 {
        f32::abs(self.max.x - self.min.x) * f32::abs(self.max.y - self.min.y) * f32::abs(self.max.z - self.min.z)
    }
}
...
mod tests {
    ...
    fn test_bounding_box3d_volume() {
        let bbox = BoundingBox3d::from_min_max(Vector3::new(0.0, 0.0, 0.0), Vector3::new(1.0, 1.0, 1.0));
        assert_eq!(bbox.volume(), 1.0);

        let bbox = BoundingBox3d::from_min_max(Vector3::new(0.0, 0.0, 0.0), Vector3::new(2.0, 2.0, 2.0));
        assert_eq!(bbox.volume(), 8.0);

        let bbox = BoundingBox3d::from_min_max(Vector3::new(-1.0, 2.0, 3.0), Vector3::new(1.0, 3.0, 4.0));
    ...
    fn test_bounding_box3d_contains() {
        ...
        let bbox = BoundingBox3d::from_min_max(Vector3::new(-1.0, -3.5, 0.0), Vector3::new(2.0, 2.0, 2.0));
    ...
    fn test_bounding_box3d_vertices() {
        ...
        assert_eq!(vertices[1], Vector3::new(bbox.min.x, bbox.min.y, bbox.max.z));
        assert_eq!(vertices[2], Vector3::new(bbox.max.x, bbox.min.y, bbox.max.z));
        assert_eq!(vertices[3], Vector3::new(bbox.max.x, bbox.min.y, bbox.min.z));
        assert_eq!(vertices[4], Vector3::new(bbox.min.x, bbox.max.y, bbox.min.z));
        assert_eq!(vertices[5], Vector3::new(bbox.min.x, bbox.max.y, bbox.max.z));
        assert_eq!(vertices[6], bbox.max);
        assert_eq!(vertices[7], Vector3::new(bbox.max.x, bbox.max.y, bbox.min.z));
    }

    #[test]
    fn test_bounding_box3d_intersects() {
        let bbox1 = BoundingBox3d::from_min_max(Vector3::new(0.0, 0.0, 0.0), Vector3::new(1.0, 1.0, 1.0));
        let bbox2 = BoundingBox3d::from_min_max(Vector3::new(0.5, 0.5, 0.5), Vector3::new(1.5, 1.5, 1.5));
        assert!(bbox1.intersects(&bbox2));

        let bbox1 = BoundingBox3d::from_min_max(Vector3::new(0.0, 0.0, 0.0), Vector3::new(1.0, 1.0, 1.0));
        let bbox2 = BoundingBox3d::from_min_max(Vector3::new(1.5, 0.5, 0.5), Vector3::new(2.5, 1.5, 1.5));
        assert!(!bbox1.intersects(&bbox2));
    }
}
