...
use factorgraph::{bbox::BoundingBox2d, robot::{CommunicationMedia, Meter, Pose2d, Position2d, Probability, Robot, RobotState, Velocity2d}, FactorGraph};

fn create_robots() -> Vec<Robot<BoundingBox2d>> {
        ...
        let initial_state = RobotState {
                pose: Pose2d {
                    position: Position2d::new(
                        initial_position.x + i as f64 * 2.0,
                        0.0,
                    ),
                    orientation: 0.0,
                },
...
fn main() {
    let mut robots: Vec<Robot<BoundingBox2d>> = create_robots();
