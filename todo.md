...
- We want to implement reset environment API call. And a load environment giving a existing environment name, so it will load the default config. This is already supported by the UI, so we just need to mirror that logic.
