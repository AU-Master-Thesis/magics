...
use gbp_config::{Config, FormationGroup, RRTSection};
use gbp_environment::Environment;
use gbp_global_planner::rrtstar::spawn_pathfinding_task_full_tree;
use magics::{
...
// use magics::theme::ColorFromCatppuccinColourExt;
use parry2d::{
...
/// **Bevy** marker [`Component`] for every entity who wants to find a path
...
fn rrt_path(
    ...
    // query all pathfinders
    pathfinders: Query<Entity, With<Pathfinder>>,
    mut tasks: Query<&mut PathfindingTask>,
    colliders: Res<Colliders>,
    config: Res<Config>,
) {
    for pathfinder in pathfinders.iter() {
        spawn_pathfinding_task_full_tree(
            &mut commands,
            START.xy(),
            END.xy(),
            config.rrt.clone(),
